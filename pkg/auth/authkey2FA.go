...
type tfa struct {
	weaver.Implements[TwoFactorAuthentication]
}

func (t *tfa) GenerateAuthKey2FA(ctx context.Context) (string, error) {
	log := t.Logger(ctx)
...
func (t *tfa) VerifyAuthKey2FA(ctx context.Context, authkey_hash, input string) (bool, error) {
	log := t.Logger(ctx)
	var mu sync.Mutex
	mu.Lock()
	defer mu.Unlock()
