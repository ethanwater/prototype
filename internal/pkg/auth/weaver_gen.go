...
func (s t_local_stub) GenerateAuthKey2FA(ctx context.Context) (r0 string, err error) {
	...
	if span.SpanContext().IsValid() {
		...
		ctx, span = s.tracer.Start(ctx, "auth.T.GenerateAuthKey2FA", trace.WithSpanKind(trace.SpanKindInternal))
		defer func() {
			if err != nil {
...
func (s t_local_stub) VerifyAuthKey2FA(ctx context.Context, a0 string, a1 string) (r0 bool, err error) {
	...
	if span.SpanContext().IsValid() {
		// Create a child span for this method.
		ctx, span = s.tracer.Start(ctx, "auth.T.VerifyAuthKey2FA", trace.WithSpanKind(trace.SpanKindInternal))
		defer func() {
			if err != nil {
				span.RecordError(err)
				span.SetStatus(codes.Error, err.Error())
...
func (s t_client_stub) GenerateAuthKey2FA(ctx context.Context) (r0 string, err error) {
	...
	if span.SpanContext().IsValid() {
		// Create a child span for this method.
		ctx, span = s.stub.Tracer().Start(ctx, "auth.T.GenerateAuthKey2FA", trace.WithSpanKind(trace.SpanKindClient))
	}
...
func (s t_client_stub) VerifyAuthKey2FA(ctx context.Context, a0 string, a1 string) (r0 bool, err error) {
	...
	if span.SpanContext().IsValid() {
		// Create a child span for this method.
		ctx, span = s.stub.Tracer().Start(ctx, "auth.T.VerifyAuthKey2FA", trace.WithSpanKind(trace.SpanKindClient))
	}
